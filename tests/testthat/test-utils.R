...
## FIND DATA FREQUENCY ================
context("Find frequency of data")
...
context("Regularise time series")

non_missing <- tibble(dates = quarterly, y1 = rnorm(10), y2 = rnorm(10))
missing <- non_missing[c(1:5,7:10), ]

expect_equal(regularise(non_missing, date_col = "dates", frequency = "quarter", group_vars = NULL), non_missing) # should be no-op

shouldbe <- non_missing
shouldbe[6, c("y1", "y2")] <- NA
expect_equal(regularise(missing, date_col = "dates", frequency = "quarter", group_vars = NULL), shouldbe)

# Wrongly passing in ungrouped data, so that there are duplicate groups
duplicate <- bind_rows(missing, non_missing)
expect_error(regularise(duplicate, date_col = "dates", frequency = "quarter", group_vars = NULL))
...
expect_false(any(is.na(regularise(grouped_missing, date_col = "dates", frequency = "quarter", group_vars = "group")$group)))
